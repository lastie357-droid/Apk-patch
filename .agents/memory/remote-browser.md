...
download is complete. `Runtime.evaluate` values are returned directly under
the CDP result's `result.value` key.

**Why:** Using the browser socket makes page commands fail with “Page.enable
wasn't found”, reading runtime values too deeply silently drops valid DOM
matches, and processing on first appearance can copy a partial APK.
